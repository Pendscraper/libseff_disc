...
DEFINE_EFFECT(prime, bool, { int64_t e; });
...
static int64_t handle_toplevel(seff_coroutine_t *k) {
  seff_request_t req = seff_resume(k, NULL, HANDLES(EFF_ID(prime)));
  int64_t result = 0;
  bool done = false;
  while (!done) {
    CASE_SWITCH(req, {
      CASE_EFFECT(prime, {
        req = seff_resume(k, (void*) true, HANDLES(EFF_ID(prime)));
        break;
      });
      CASE_RETURN({
        done = true;
  ...
  while (!done) {
    CASE_SWITCH(req, {
      CASE_EFFECT(prime, {
        ...
        req = seff_resume(k, (void*) res, HANDLES(EFF_ID(prime)));
        break;
      });
      CASE_RETURN({
        done = true;
