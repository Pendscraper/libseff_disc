...
MAKE_SYSCALL_WRAPPER(seff_coroutine_t *, seff_coroutine_new, seff_start_fun_t *, void *);
MAKE_SYSCALL_WRAPPER(void, seff_coroutine_delete, seff_coroutine_t *);
MAKE_SYSCALL_WRAPPER(void, syscall, void);
...
void *skynet(void *_arg) {
    ...
    if (num >= last_layer) {
        ...
        syscall_syscall_wrapper();
    } else {
        num *= 10;
        for (size_t i = 0; i < 10; i++) {
            children[i] = seff_coroutine_new_syscall_wrapper(skynet, (void *)(num + i));
        }
        size_t finished = 0;
        while (finished < 10) {
            for (size_t i = 0; i < 10; i++) {
...
int main(int argc, char **argv) {
    ...
    seff_request_t request = seff_resume(root, NULL, EFF_ID(yield_int));
    while (true) {
        ...
        request = seff_resume(root, NULL, EFF_ID(yield_int));
    }
}
